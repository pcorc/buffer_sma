"""
BATCH 7 COMPLETE PLOTTER - WITH BAR CHARTS + TABLE
===================================================

Creates:
1. Time series with regime shading (all 3 strategies + benchmarks)
2. Bar chart: Performance by regime (Bullish/Neutral/Bearish)
3. Bar chart: ECR outperformance by regime
4. Comprehensive regime performance table (CSV + console)

Run: python plot_batch_7_complete.py
"""
...
def load_csv(csv_path):
    ...
    for col in df.columns:
        if 'Regime_Adaptive' in col or 'regime_adaptive' in col:
            available_strategies['Regime_Adaptive'] = True
    ...
    print(f"  {'Regime-Adaptive ECR:':<30} {'✅ YES' if available_strategies['Regime_Adaptive'] else '❌ NO'}")
    ...
    if not available_strategies['cap_moderate'] or not available_strategies['Existing_90']:
        ...
        print(f"   Plots will only show available strategies.")
        print(f"   Run complete Batch 7 for all strategies.")
...
def find_strategy_column(df, month, strategy_keywords):
    ...
    strategy_keywords can be a list like ['Regime_Adaptive'] or ['Existing_90', 'Existing', '90']
    """
...
def create_regime_shaded_plot(df, df_regimes, month, output_dir, available_strategies):
    """Create time series with regime shading - only plot available strategies."""
    ...
    ax.set_title(
        f'Batch 7: Regime-Adaptive Performance\n{month} Launch Month\n'
    ...
    plot_path = output_dir / f'batch_7_regime_shaded_{month}.png'
...
def calculate_regime_stats(df, df_regimes, months):
    ...
    for month in months:
        # Find return columns
        ecr_col = find_strategy_column(df_merged, month, ['Regime_Adaptive', 'regime_adaptive'])
        ...
        for regime in [-1, 0, 1]:
            ...
            if ecr_col and ecr_col in df_merged.columns:
                ...
                if len(returns) > 0:
                    results['Regime_Adaptive'][regime].append({
                        'month': month,
...
def create_bar_charts_and_table(regime_stats, output_dir):
    ...
    for regime in [1, 0, -1]:
        ecr_stats = regime_stats['Regime_Adaptive'].get(regime, [])
    ...
    # PRINT TABLE
    print(f"\n{'='*130}")
    print(f"REGIME PERFORMANCE ANALYSIS TABLE")
    print(f"{'='*130}\n")
    ...
    csv_path = output_dir / 'batch_7_regime_performance_table.csv'
    ...
    ax1.bar(x - width, ecr_values, width, label='Regime-Adaptive ECR', color='darkblue', alpha=0.8)
    ...
    ax1.set_title('Performance During Different Market Regimes', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ...
    ax2.set_title('Regime-Adaptive Outperformance by Regime', fontsize=14, fontweight='bold')
    ax2.set_xticks(x)
    ...
    plot_path = output_dir / 'batch_7_regime_bar_charts.png'
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    """Main execution."""
    print("="*80)
    print("BATCH 7 COMPLETE PLOTTER")
    print("="*80)
    ...
    csv_path = Path('output/backtest_results/batch_7/batch_7_daily_time_series.csv')
    df, months, available_strategies = load_csv(csv_path)  # Now returns 3 items
