...
def create_comparison_plot(df, output_path):
    """Create 4-line comparison plot with statistics in legend."""
    ...
    # Plot each strategy with statistics in label
    colors = {
        'SPY': ('black', '-', 2.0, 0.7),
        'BUFR': ('gray', '--', 2.0, 0.7),
        'ECR_V2': ('blue', '-', 2.5, 1.0),
        'Existing_90': ('red', '-', 2.5, 1.0)
    }

    labels_map = {
        'SPY': 'SPY (Buy & Hold)',
        'BUFR': 'BUFR (Benchmark)',
        'ECR_V2': 'ECR V2 (Quarterly, Equal Weights)',
        'Existing_90': 'Existing (90% Cap Threshold)'
    }

    for prefix, (color, style, width, alpha) in colors.items():
        col_name = f'{prefix}_NAV_normalized'
        if col_name in df.columns and prefix in stats_dict:
            stats = stats_dict[prefix]

            # Create label with statistics
            label = (f"{labels_map[prefix]}\n"
                    f"  Return: {stats['total_return']*100:>6.2f}% | "
                    f"Ann: {stats['ann_return']*100:>6.2f}% | "
                    f"Sharpe: {stats['sharpe']:>4.2f} | "
                    f"MaxDD: {stats['max_dd']*100:>5.1f}%")

            ax.plot(df['Date'], df[col_name],
                   label=label, color=color, linestyle=style,
                   linewidth=width, alpha=alpha)
            print(f"  ✅ Plotted: {labels_map[prefix]}")
    ...
    # Legend with statistics - place outside plot area
